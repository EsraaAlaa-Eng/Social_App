...
export const generateToken = async ({
    ...
    options = { expiresIn: Number(process.env.ACCESS_TOKEN_EXPIRES_IN || 3600) },
...
export const detectSignatureless = async (role: RoleEnum = RoleEnum.user): Promise<signatureLevelEnum> => {
...
export const createLoginCredentials = async (user: HUserDocument) => {
    const signatureLevel = await detectSignatureless(user.role)
